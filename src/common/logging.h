 ...
 * }
 */
#ifndef F2M_COMMON_LOGGING_H_
#define F2M_COMMON_LOGGING_H_
...
//    to choose an output stream, outputs a message head into the stream and flush.
//  - The std::ostream reference is returned by LoggerStart(), passed to user-speific
//    output operators (<<), which writes the log message boby.
...
// head because the time when the destructor is invoked 
...
// destructed, the destructor might not have the chance to append its flush flags. 
// Without flush in Logger::Start(), this may cause the lose of the last few messages. 
// However, given flush in Start(), program crashing between invocations to Logger::Start() and
// destructor only causes the lose of the last message body, while the message head 
// will be there.
#define LOG(severity)        \
  Logger(severity).Start(severity, __FILE__, __LINE__, __FUNCTION__)

#endif // F2M_COMMON_LOGGING_H_
